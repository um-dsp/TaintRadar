class NavexMain(val cpg: Cpg) {
    ...
    def outputPaths(debug:Boolean = true) = {
        ...
        output #> ("/home/umd-user/joern/projectcpg/paths/" + cpg.metaData.root.head.split("/").last.replaceAll("[^a-zA-Z]", "").toLowerCase + "-output.json")
    
        logger.mkString(",") #>> "/home/umd-user/joern/projectcpg/app-stats.csv"
