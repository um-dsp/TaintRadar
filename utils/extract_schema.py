...
def main():
    parser = argparse.ArgumentParser(
        description="Extract database schema (table/columns/types) from SQL files and write it as a CSV file."
    )
    parser.add_argument(
        "inputs",
        nargs="+",
    ...
    parser.add_argument(
        "--out-dir",
        default="output/db-schemas",
        help="Output directory for CSV files (default: output/db-schemas)"
    )
    parser.add_argument(
        "--out-file",
        help="Override the output CSV filename for the input SQL file."
    )
    ...
    for sql_file in sql_paths:
        try:
            ...
            df["Properties"] = df["Properties"].apply(lambda x: " ".join(x))

            file_stem = sql_file.stem
            out_csv = out_dir / (args.out_file or f"{file_stem}-database.csv")
