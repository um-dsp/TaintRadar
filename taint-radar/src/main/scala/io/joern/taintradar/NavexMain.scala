...
import io.joern.taintradar.Utils
...
class NavexMain(val cpg: Cpg,val shouldAugment: Boolean) {
    ...
    def getPaths(vulnerability: String, debug: Boolean = false) = {
        ...
        val paths: List[List[AstNode]] = sinks.map(sink => CpgUtils.reachableBySources(sink, sources, tagName)).filterNot(_.isEmpty)
    ...
    def outputPaths(debug: Boolean = true) = {
        ...
        // Write JSON output
        val jsonPath = Paths.get("taint-radar/output/paths/" + 
        ...
        // Append stats to CSV
        val csvPath = Paths.get("taint-radar/output/stats.csv")
