...
import io.shiftleft.codepropertygraph.generated.nodes.{Call, Identifier, Literal, FieldIdentifier, AstNode}
import io.shiftleft.semanticcpg.language.*

import io.joern.taintradar.Constants.ConstantsFactory
import io.joern.taintradar.Constants.SQLConstants
...
class DatabaseConstraint(val cpg: Cpg) {
    ...
    val queryStatementsDuplicates: List[Call] = cpg.call.filter(call => Constants.query_concat_func.contains(call.name)).filter(x => mainSqlKeywords.exists(x.argument.head.code.toLowerCase.replaceFirst("^[^a-zA-Z0-9]*", "").startsWith(_))).l
    ...
    val reader = {
        val dbSchemaPath = Paths.get("taint-radar/db-schemas/" + file_name + "-database.csv")
        val emptySchemaPath = Paths.get("taint-radar/db-schemas/empty-database.csv")
        
        if (Files.exists(dbSchemaPath)) {
            CSVReader.open(dbSchemaPath.toString)
        } else {
            println("No database schema found for " + file_name + ". Using empty database schema.")
            if (!Files.exists(emptySchemaPath)) {
                Files.createDirectories(emptySchemaPath.getParent)
                Files.createFile(emptySchemaPath)
            }
            CSVReader.open(emptySchemaPath.toString)
        }
    }
    ...
    def augmentDbCalls() = {
        if (tables.isEmpty) println("No database to parse")
            ...
            val pass = new CpgPass(cpg) {
                override val name = "DatabaseConstraint"
    ...
    def debug() = {
        ...
        val outputPath = Paths.get("taint-radar/output/db-schemas/" + file_name + ".csv")
