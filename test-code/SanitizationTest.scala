object SanitizationFilter {
   ...
   def isSanitized(node: Any, sanitizedParameters: List[Boolean] = List())(implicit san_functions_specific: List[String]): Boolean = node match {
      ...
      case identifier: Identifier => {
         val isArgumentSanitized = sanitizedParameters
         val definingNode = {
            ...
            // identifier passed by reference to function
            else if (identifier.astParent.isCallTo(".*").callee.parameter.l.map(p => p.evaluationStrategy == "BY_REFERENCE")(identifier.order-1)) {
               // val paramDdgIn: Identifier = identifier.ddgIn.isIdentifier.l.head
               ...
               val isArgumentSanitized: List[Boolean] = identifier.astParent.isCallTo(".*").argument.l.map(isSanitized(_, sanitizedParameters)(san_functions_specific))
               identifier.astParent.isCallTo(".*").callee.methodReturn.ddgIn.isIdentifier.name(identifier.astParent.isCallTo(".*").callee.parameter.l(identifier.order-1).name)
            }
            // empty ddgIn edge
