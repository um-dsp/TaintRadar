object SanitizationFilter {
   ...
   */
   ...
   def setCpg(cpgInput: Cpg) = {
      ...
      val values: List[List[Expression]] = constants.map(constant => _cpg.get.call("define").filter(_.argument(1).code.replace("\"", "") == constant).argument(2).l) 
      constantTable = Some((constants zip values).toMap[String, List[Expression]])
      
   }   

   def isMethodSanitized(function: Call, arguments: List[Expression], sanitizedParameters: List[Boolean])(implicit vulnerabilityInst: vulnerabilityType): Boolean = {
      ...
      val method: Method = {
         ...
         else _cpg.get.method(function.name).filter(_.code!="<empty>").head
      }
      ...
      else if (function.dispatchType == "DYNAMIC_DISPATCH" && _cpg.get.method(function.name).filter(_.code!="<empty>").size > 1) false
         ...
         try { 
            ...
            case identifier: Identifier => {
               ...
               val definingNode = {
                  ...
                  //    val includeNodes = _cpg.get.method("<global>").where(_.namespaceBlock.fullName.filter(_==identifier.file.namespaceBlock.fullName.head)).call("include|require").l
   ...
   def printTestOutput(cpg: Cpg)(implicit vulnerabilityInst: vulnerabilityType = vulnerabilityType(name = "", sanitization_functions = List())) = {
      val t0 = System.nanoTime()
      // val san_functions = san_functions_specific
      SanitizationFilter.setCpg(cpg)
      val sanitized = cpg.identifier.filter(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).name.dedup.l.filter(!List("p1", "p2", "unsan11", "unsan14").contains(_))
      val unsanitized = cpg.identifier.filterNot(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).name.dedup.l.filter(!List("p1", "p2", "san12", "san6", "tmp", "_GET", "san14").contains(_))
      val sanitizedConstants = cpg.method.ast.isFieldIdentifier.filter(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).canonicalName.dedup.l
      val unsanitizedConstants = cpg.method.ast.isFieldIdentifier.filterNot(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).canonicalName.dedup.l
      // val sanitized = cpg.identifier.filter(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).name.dedup.l
      // val unsanitized = cpg.identifier.filterNot(SanitizationFilter.isSanitized(_)(vulnerabilityInst)).name.dedup.l
